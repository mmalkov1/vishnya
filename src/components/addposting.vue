    ...
    <div class="container">
      ...
      <div class="order-body row col-12 mt-4">
        ...
        <table class="table table-striped">
          ...
          <tbody>
            <tr
              v-for="(product, index) in postings.products"
              :key="product.id"
              class="align-middle"
            >
              ...
              <td class="product__orderquant">
                <input type="text" v-model="product.product_orderquant" class="form-control">
              </td>
              <td class="align-middle">{{product.product_count}}</td>
              <td class="product__price">
                <input type="text" v-model="product.product_price" class="form-control">
              </td>
              <td
                class="align-middle"
              >{{productSum(product.product_orderquant, product.product_price, index)}}</td>
              <td class="options d-flex">
                ...
                />
                <button class="btn btn-outline-danger" @click="deleteProduct" :data-id="index">
        ...
        <button class="btn btn-outline-info" @click="addPostingProduct">
...
export default {
  ...
  computed: {
    total: function() {
      return this.postings.products.reduce(
        (acc, el) => acc + el.product_sum,
        0
      );
    }
  },
  methods: {
    ...
    getProduct(id, index) {
      ...
      axios({
        ...
        .then(res => {
          newProduct.product_residue = res.data.reduce(
      ...
      axios({
        ...
        .then(res => {
          ...
          if (catchDocument && catchDocument === "catchPosting") {
            ...
            this.postings.products.map(
              el => (el.product_count = el.product_orderquant)
            );
