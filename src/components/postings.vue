<template>
  <div>
    <appHeader/>
    <div class="container" v-if="this.$store.state.userAuth">
      ...
      <div class="row col-12 mt-3">
        ...
        <div class="form-group col-5 ml-auto row">
          <div class="col-5 ml-auto">
            <router-link class="btn btn-success" to="postings/add">Добавить</router-link>
          </div>
        </div>
      </div>
          ...
          <tr v-for="posting in orderedPosting" :key="posting.id">
            ...
            <td>
              <router-link
                ...
                :to="{name: 'postings', params: { id: posting.id}}"
              >
...
export default {
  ...
  computed: {
    ...
    orderedPosting: function() {
      if (!this.filter) {
        return _.slice(
          _.orderBy(this.postings, this.sort, this.ordered),
        ...
        return _.slice(
          _.orderBy(
            _.filter(this.postings, o => {
              return (
                o.number == this.filter ||
                _.includes(o.provider, this.filter) ||
                _.includes(o.status, this.filter)
              );
            }),
            this.sort,
            this.ordered
