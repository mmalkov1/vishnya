<template>
  <div>
    <appHeader></appHeader>
    <div class="container">
      <h1>Приход товара № {{this.$route.params.id}}</h1>
      ...
      <div class="order-body row col-12 mt-4">
          ...
          <tbody>
            <tr class="align-middle" v-for="(product,index) in products" :key="index">
              ...
              <td class="product__price">
                ...
                >
                <span v-show="!showElement">{{isShownValue(product.product_price)}}</span>
              </td>
              <td
                class="align-middle"
              >{{isShownValue(totalProduct(product.product_orderquant, product.product_price, index))}}</td>
              <td class="options d-flex">
                <modalPrice
                  class="mr-1"
                  :productId="product.product_id"
                  :postingCount="product.product_orderquant"
                  :postingPrice="product.product_price"
                />
                <button
        ...
        <button class="btn btn-outline-info" v-show="showElement" @click="addPostingProduct">
      ...
      <div class="col-12 mt-5">
        ...
        <button
          ...
          v-show="!showElement"
          @click="cancelCatchPosting"
        >Отменить проведение</button>
  ...
  methods: {
    ...
    getProduct(id, index) {
      ...
      axios({
        ...
        .then(res => {
          newProduct.product_count = res.data.reduce(
            (acc, el) => acc + el.product_count,
            0
          );
        })
    ...
    getPosting() {
      axios
        .get(`${this.$store.state.host}/postings/id/${this.$route.params.id}`)
        .then(res => {
    ...
    getPostingProducts() {
      axios
        .get(
          `${this.$store.state.host}/orderproducts/id/1/${
    ...
    cancelCatchPosting() {
      ...
      axios({
        method: "PUT",
        url: `${this.$store.state.host}/orderproducts/id/1/${
          this.$route.params.id
        }`,
        data: this.products
    ...
    catchPosting() {
      ...
      axios({
        method: "PUT",
        url: `${this.$store.state.host}/postings/id/${this.$route.params.id}`,
    ...
    getProductCatch(id) {
      return (
        this.counts
          .filter(el => el.product_id == id && el.documentStatus !== 1)
          .reduce((acc, item) => acc + item.product_count, 0) * -1
      );
    },
    calculateProductTotal() {
      for (let elem of this.arrProductId) {
        ...
        if (total < 1) {
          axios
            .get(`${this.$store.state.host}/products/id/${elem}`)
            .then(prod => {
              let option = {
              ...
              axios(
                `https://vishnya.org.ua/wp-json/wc/v2/products/${
                  prod.data.wc_id
                }`,
                option
              )
