<template>
  <div class="container">
    ...
    <div class="order-head row col-12 mt-2 mb-2">
      ...
      <div class="form-group col-4">
        ...
        <select class="custom-select" v-model="order.delType">
          <option v-for="type in deliveryType" :key="'type'+type.id">{{type.name}}</option>
        </select>
      </div>
    </div>
    <hr>
    <div class="order-body row col-12 mt-4">
      ...
      <button class="btn btn-outline-info" @click="addOrderProduct">
    ...
    <div class="col-12 mt-5">
      ...
      <router-link class="btn btn-light" to="/orders">Назад</router-link>
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    getProduct(id, index) {
      ...
      newProduct.product_price = id.acc_product_prices.find(
        el => el.accPriceTypeId == 2
      ).price_count; //цена розница
      ...
      axios({
        ...
        .then(res => {
          ...
          if (catchDocument && catchDocument === "catchOrder") {
            ...
            this.order.products.map(
              el => (el.product_count = el.product_orderquant * -1)
            );
          }
          axios({
            method: "POST",
